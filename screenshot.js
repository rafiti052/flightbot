...
if (!route) { console.error("No active route found."); process.exit(1); }

function buildUrl(r) {
  const query = r.roundTrip
    ? `Round-trip ${r.from} to ${r.to} ${r.departureDate} return ${r.returnDate}`
    : `One-way ${r.from} to ${r.to} ${r.departureDate}`;
  return `https://www.google.com/travel/flights/search?q=${encodeURIComponent(query)}&curr=${r.currency ?? "USD"}&hl=en`;
}
...
for (const sel of ['button[aria-label="Accept all"]', 'button[aria-label="Reject all"]', '[jsname="b3VHJd"]', '.tHlp8d button']) {
  try { const el = await page.$(sel); if (el) { await el.click(); console.log(`Dismissed: ${sel}`); } } catch {}
}
