...
**Load confirmed lessons first:** Run `pnpm exec tsx <skill-dir>/scripts/lessons.ts list --status confirmed` before clarifying.
...
**Rules:** one requirement per criterion; use concrete values; every criterion contains a SHALL and is measurable. `pnpm exec tsx <skill-dir>/scripts/validate_spec.ts` flags structural problems.
...
**Deterministic backing:** `pnpm exec tsx <skill-dir>/scripts/validate_spec.ts <spec-path-or-feature>` checks the artifact before confirmation.
...
- **Confirm after the gate passes** - Present the spec only after `validate_spec.ts` exits clean.
